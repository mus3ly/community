...
$(document).ready(function() {
  ...
  rangeSlider.noUiSlider.on('update', function(values, handle) {
    document.getElementById('slider-range-value1').innerHTML = parseInt(values[0])+' <?= currency() ?>';
    document.getElementById('slider-range-value2').innerHTML = parseInt(values[1])+' <?= currency() ?>';
    $('#min-value').val(parseInt(values[0]));
    $('#max-value').val(parseInt(values[1]));
    do_product_search('0');
  });
});
                    <?php
  ...
  // Set visual min and max values and also update value hidden form inputs
  rangeSlider.noUiSlider.on('update', function(values, handle) {
    document.getElementById('slider-range-value1').innerHTML = parseInt(values[0])+' KM';
    document.getElementById('slider-range-value2').innerHTML = parseInt(values[1])+' KM';
    $('#min-value').val(parseInt(values[0]));
    $('#max-value').val(parseInt(values[1]));
    do_product_search('0');
  });
	...
	function search_location()
{
    ...
    if(str.length >= 2 )
    {
        ...
        success: function(res) {
            ...
            $('#map_search #result_loc').show();
            $('#map_search #result_loc').html(res);
            // alert(res);
        }
    });
...
?>
    ...
    <input type="hidden" name="job_hours" id="job_hours" value="">
    <input type="hidden" name="event_date" id="event_date" value="">
    <input type="hidden" name="event_type" id="event_type" value="">
    <input type="hidden" name="age_restriction" id="age_restriction" value="">
    <input type="hidden" name="job_type" id="job_type" value="">
    <input type="hidden" name="modelf" id="modelf" value="">
    ...
    <input type="hidden" name="lat" id="lat" value="<?= (isset($_GET['lat'])?$_GET['lat']:'') ?>">
    <input type="hidden" name="place_id" id="place_id" value="<?= (isset($_GET['place_id'])?$_GET['place_id']:'') ?>">
    <input type="hidden" name="lng" id="lng" value="<?= (isset($_GET['lng'])?$_GET['lng']:'') ?>">
...
<script>
	...
	$("input[type='radio']").click(function(){
	    if($("input[type='radio']:checked").val() != 'blog'){
        }
	});
    ...
    function do_product_search(page){
        ...
        if($("input[type='radio']:checked").val() == 'shop_listing')
        {
            $('#map_div').hide();
            $('#content').removeClass("col-md-6");
            $('#content').addClass("col-md-9");
        } else if($("input[type='radio']:checked").val() == 'blog'){
        // alert();
    ...
    console.log($(this).val()+'-'+i);
        ...
        $('#job_hours').val($('#select_job_hours').val());
        $('#event_date').val($('#event_date_input').val());
        $('#event_type').val($('#event_type_input').val());
        $('#age_restriction').val($('#age_restriction_input').val());
        $('#job_type').val($('#select_job_type').val());
        $('#modelf').val($('#modelf_input').val());
        $('#seats').val($('#seats_input').val());
        $('#modelt').val($('#modelt_input').val());
